...
import { Cpu, Shuffle, Zap, Sync, ListChecks } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";

/**
 * AI Command Center - v4.6
 * Path: src/app/dashboard/super-admin/ai-command-center/page.js
 */
...
export default function AICommandCenter() {
  const [running, setRunning] = useState(null); // key of running command
  ...
  // runs a simulated command with steps and progress
  async function runCommand(cmdKey) {
    ...
    // call NeuroEdge mock for a summary/explanation
    try {
            ...
            <div className="col-span-2 space-y-4">
                  ...
                  return (
                    <motion.div
                      ...
                      className={`p-4 rounded-2xl border shadow-md ${
                        active ? "bg-white/90 border-green-200" : "bg-white/70 border-gray-200"
                      }`}
                    >
                      <div className="flex items-start justify-between">
                        ...
                        <div className="flex flex-col items-end">
                          <div className="text-xs text-gray-500">{/* status placeholder */}</div>
                          <div className="mt-2 flex items-center gap-2">
                            <button
                              disabled={busy && !active}
                              onClick={() => runCommand(cmd.key)}
                              className={`px-3 py-2 rounded-md font-medium ${
                                active ? "bg-green-600 text-white" : "bg-indigo-600 text-white"
                              } disabled:opacity-50`}
                            >
                              {active ? "Running…" : "Run"}
                            </button>
                            <button
                              onClick={() => {
                                // quick simulate: append log and brief AI note
                                appendLog(`Inspect ${cmd.label}`);
                                setAiNote(`Quick inspect for ${cmd.label}: nominal (mock).`);
                              }}
              ...
              <div className="bg-white/70 p-4 rounded-2xl shadow-md">
                ...
                <div className="max-h-48 overflow-auto font-mono text-xs text-gray-700 bg-white/30 p-2 rounded">
                  <AnimatePresence>
                    {log.length === 0 && <div className="text-gray-400">No logs yet — run a command to start</div>}
                    {log.map((l, i) => (
                      <motion.div key={i} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="py-1 border-b last:border-b-0">
                        {l}
                      </motion.div>
                    ))}
                  </AnimatePresence>
              ...
              <div className="bg-gradient-to-r from-indigo-50 to-purple-50 p-4 rounded-2xl shadow-md">
                ...
                <div className="mt-3 text-gray-700 min-h-[64px]">
                  {aiNote || "No AI notes yet. Run a command to get NeuroEdge commentary."}
                </div>
              </div>
            </div>
            ...
            <aside className="space-y-4">
              <div className="p-4 bg-white/80 rounded-2xl shadow-md">
                ...
                <div className="mt-3 space-y-2">
                  <button
                    onClick={() => {
                      runCommand("system_sync");
                    }}
                    className="w-full text-left px-3 py-2 bg-indigo-600 text-white rounded-md"
                  >
                    <div className="flex items-center gap-2"><SyncIcon /> Trigger Full Sync</div>
                  </button>

                  <button
                    onClick={() => {
                      runCommand("emergency_drill");
                    }}
                    className="w-full text-left px-3 py-2 border rounded-md"
                  >
                    <div className="flex items-center gap-2"><ZapIcon /> Run Emergency Drill</div>
                  </button>
                </div>
              </div>
                ...
                <div className="space-y-2">
                  ...
                  {history.map((h) => (
                    <div key={h.id} className="p-2 rounded border">
                      ...
                      <div className="text-xs text-gray-700 mt-1">{h.summary?.slice(0, 100)}</div>
                    </div>
                  ))}
                </div>
              </div>
...
// small icons inline so we don't add imports
function SyncIcon() { return <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M21 12a9 9 0 10-3.6 7.2" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"></path><path d="M21 3v6h-6" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"></path></svg>; }
function ZapIcon(){ return <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M13 2L3 14h7l-1 8 10-12h-7l1-8z" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/></svg>; }
